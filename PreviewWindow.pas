...
uses
  ...
  System.Classes, Vcl.Graphics, System.SyncObjs, System.Generics.Collections,
...
begin
  if assigned(m_deckLinkScreenPreviewHelper) then
    m_deckLinkScreenPreviewHelper := nil;
...
begin
  ...
  if (CoCreateInstance(CLASS_CDeckLinkGLScreenPreviewHelper, nil, CLSCTX_ALL,
    IID_IDeckLinkGLScreenPreviewHelper, m_deckLinkScreenPreviewHelper) <> S_OK)
  then
    exit;
...
begin
  ...
  if ((not assigned(m_deckLinkScreenPreviewHelper)) or (m_previewBoxDC = 0) or
    (m_openGLctx = 0)) then
  begin
    exit(E_FAIL);
  end;
